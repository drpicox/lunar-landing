/**
 *
 * GogoMakePlay Game Engine v1.7.3
 * Copyright (c) 2010 Trevor Cowley
...
G.makeGob = function (id, parentGob, tagName, parentTag) {  
    if (G.O[id]) return G.O[id]; // no duplicates
...
G.deleteGob = function (id) { 
    ...
    G.O[id].tag.parentNode.removeChild(G.O[id].tag); // remove the Gob's tag from the DOM
...
G.loadCookies = function () {  
    ...
    var i=0, arr = document.cookie.split(';'), re = new RegExp(unescape(G.cookieId) + '=');
...
G.Gob.prototype.fixIntersection=function(gob) {  // requires same parent gob. back up just outside given gob
    var i=0, t=this, dir=G.dirXY(t.nx-t.lx,t.ny-t.ly); 
...
G.KB.addKeys = function () {  
    var i, arr = Array.prototype.slice.call(arguments);
    for (i = 0; i<arr.length; i++) {    
...
G.KB.removeAllKeys = function () {  
    for (keystr in this.keys) delete this[keystr];
    this.keys = {};
    this.codes = [];
    return this;
};
